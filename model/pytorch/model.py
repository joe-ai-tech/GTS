...
class GTSModel(nn.Module, Seq2SeqAttrs):
    ...
    def forward(self, label, inputs, node_feas, temp, gumbel_soft, labels=None, batches_seen=None, save_adj_name=None):
        """
        ...
        if save_adj_name != None:
            ###########
            # Save the latent graph
            y_temp = F.softmax(x, dim=-1)
            output_adj_cont = y_temp[:, 0].clone().reshape(self.num_nodes, -1).cpu().detach().numpy()
            shape = x.size()
            _, k = y_temp.data.max(-1)
            y_hard = torch.zeros(shape).to(device)
            y_hard = y_hard.zero_().scatter_(-1, k.view(shape[:-1] + (1,)), 1.0)
            y_temp = torch.autograd.Variable(y_hard - y_temp.data) + y_temp
            output_adj = y_temp[:, 0].clone().reshape(self.num_nodes, -1).cpu().detach().numpy()

            csv_file = 'adj_cont_' + str(save_adj_name) + '.csv'
            np.savetxt(csv_file, output_adj_cont, delimiter=",")
            csv_file = 'adj_' + str(save_adj_name) + '.csv'
            np.savetxt(csv_file, output_adj, delimiter=",")
